<?php
    ...
    $initial_display=$change['Selected'];
    $text_box=$radio_button=$dropdown_menu=$sun_url=$sun_json=$sun_array=$counter=$res=$dets=$clear=$tbo=$cha="";
    $output="<br>";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['kill'])) {
            ...
            $radio_button=$_SESSION['rad1'];
            $text_box=$statecode[$_POST['new'][1]];        
            $bio=$_POST['new'][0];
            $sun_url = 'http://congress.api.sunlightfoundation.com/legislators?chamber='.$radio_button.'&state='.$text_box.'&bioguide_id='.$bio.'&apikey=725651676ce9425d9cea2e39d3c2dc88';
            ...
            if (!empty($_POST["tebo"])) {
                $text_box = trim($_POST["tebo"]);
                if (!empty($_POST["SH"])) {
                    $radio_button = $_POST["SH"];
                    if (!empty($_POST["selop"])) { 
                        $dropdown_menu = $_POST["selop"];
                            
                        $initial_display="";
                        if(strlen($statecode[ucfirst(strtolower($text_box))])==0) {
                            $names = explode(" ", $text_box);
                            if(count($names) >= 2)
                            {
                            ...
                            $sun_url = "http://congress.api.sunlightfoundation.com/legislators?chamber=".$radio_button;
                            if(isset($first_name))
                            ...
                            if(strlen($text_box)>2) {
                                $tbo = $statecode[ucfirst(strtolower($text_box))];
                            }
                            $sun_url = 'http://congress.api.sunlightfoundation.com/legislators?chamber='.$radio_button.'&state='.$tbo.'&apikey=725651676ce9425d9cea2e39d3c2dc88';
                        }
                    
                        $_SESSION['change1']=$change[$dropdown_menu];
                        $_SESSION['drop1']=$dropdown_menu;
                        $_SESSION['rad1']=$radio_button;
                        $_SESSION['tbox1']=$text_box;
                    
                        if($dropdown_menu == "legislators") {
                            $sun_json = file_get_contents($sun_url);
                            $sun_array = json_decode($sun_json,true);
                            $_SESSION['url1']=$sun_array;
                            $counter = (int)$sun_array['count'];
                            if(!$counter)
                                $output .= "The API returned zero results for the request";
                            else
                                $output .= "<form method=\"post\" action=congress.php id=\"f2\"><input type=\"hidden\" value=\" \" name=\"new[]\" id='selected_bioguideID' ><input type=\"hidden\" value=\"".$text_box."\" name=\"new[]\" ></form><table id=\"tab1\" border=1 ><tr><th>Name</th><th>State</th><th>Chamber</th><th>Details</th></tr>";
                        ...
                        if($dropdown_menu == "committees") {
                            $sun_url = 'http://congress.api.sunlightfoundation.com/committees?committee_id='.$text_box.'&chamber='.$radio_button.'&apikey=725651676ce9425d9cea2e39d3c2dc88';
                            $sun_json = file_get_contents($sun_url);
                            $sun_array = json_decode($sun_json,true);
                            $_SESSION['url1']=$sun_array;
                            $counter = (int)$sun_array['count'];
                        ...
                        if($dropdown_menu == "bills") {
                            $sun_url = 'http://congress.api.sunlightfoundation.com/bills?bill_id='.$text_box.'&chamber='.$radio_button.'&apikey=725651676ce9425d9cea2e39d3c2dc88';
                            $sun_json = file_get_contents($sun_url);
                            $sun_array = json_decode($sun_json,true);
                            $_SESSION['url1']=$sun_array;
                            $counter = (int)$sun_array['count'];
                        ...
                        if($dropdown_menu == "amendments") {
                            $sun_url = 'http://congress.api.sunlightfoundation.com/amendments?amendment_id='.$text_box.'&chamber='.$radio_button.'&apikey=725651676ce9425d9cea2e39d3c2dc88';
                            $sun_json = file_get_contents($sun_url);
                            $sun_array = json_decode($sun_json,true);
                            $_SESSION['url1']=$sun_array;
                            $counter = (int)$sun_array['count'];
    ...
    <head>
        ...
        <script>
            ...
            function myFunction() {
                ...
                document.getElementById("vc").innerHTML = changes['Selected'];                
            }
            function updatefunc() {
            ...
            <div id="maintab">
                <div id="l">
                    <center>
                        ...
                        Chamber <br>
                        <label id="vc" value="Keyword*"><?php if($initial_display==""){$cha=$_SESSION['change1']; echo $cha;} else echo $initial_display;?></label> <br>
                    </center>
                </div>
                <div id="r">
                    <center>
                        <form method="post" name="myform" id="f" action="congress.php">
                            <select id="cv" onchange="updatefunc()" name="selop" >
                                <option selected disabled value="Selected">Select your option</option>
                                <option value="legislators" <?php $dropdown_menu=$_SESSION['drop1']; echo ($dropdown_menu=="legislators")?'selected':'' ?>>Legislators</option> 
                                <option value="committees" <?php $dropdown_menu=$_SESSION['drop1']; echo ($dropdown_menu=="committees")?'selected':'' ?>>Committees</option>
                                <option value="bills" <?php $dropdown_menu=$_SESSION['drop1']; echo ($dropdown_menu=="bills")?'selected':'' ?>>Bills</option>
                                <option value="amendments" <?php $dropdown_menu=$_SESSION['drop1']; echo ($dropdown_menu=="amendments")?'selected':'' ?>>Amendments</option>
                            </select>
                            <label><input type="radio" name="SH" checked="checked" value="senate" <?php $radio_button=$_SESSION['rad1']; echo ($radio_button=="senate")?'checked':'' ?>>Senate</label>  <label><input type="radio" name="SH" value="house" <?php $radio_button=$_SESSION['rad1']; echo ($radio_button=="house")?'checked':'' ?>>House</label>
                            <input type="text" id="tb" name="tebo" value="<?php $text_box=$_SESSION['tbox1']; echo $text_box; ?>">
